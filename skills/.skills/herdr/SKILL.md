...
The `herdr` binary in `PATH` talks to the running Herdr server over its local socket. This works regardless of whether the calling process itself is inside a Herdr-managed pane — no `HERDR_ENV` or other in-pane env var is required to issue control commands. Confirm the server is actually up first:

```bash
herdr status
```

If the server isn't running, say so and stop rather than trying to control a nonexistent session.

Use the CLI to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
When this agent is itself running inside a Herdr-managed pane, Herdr injects that pane's context as env vars:

```bash
printf '%s\n' "$HERDR_WORKSPACE_ID" "$HERDR_TAB_ID" "$HERDR_PANE_ID"
```

Prefer `--current` in that case when a pane command should target the calling pane — `--current` resolves via this injected context, so it only works when these vars are set. When calling `herdr` from outside any managed pane (no `HERDR_*` vars present), there is no "current" pane: pass explicit IDs from `herdr pane list`/`herdr workspace list` instead, and skip `--current`. Omitting a target may use the UI-focused pane, which can belong to the user or another client.
...
When running inside a Herdr-managed pane, default to a sibling pane in the current tab and the current working directory. When calling from outside any managed pane, ask the user which workspace/tab to target (or use the focused one from `herdr workspace list`) since there is no caller pane to default to. Do not create a workspace, tab, worktree, or different cwd unless the user explicitly requests that topology or location.

Honor a direction requested by the user. Otherwise inspect the caller pane (in-pane case):
...
From outside a managed pane, split an explicit target pane and an explicit `--cwd` instead of `--current`/`$PWD`:

```bash
herdr pane split --pane <target-pane-id> --direction right --cwd <target-cwd> --no-focus
```
...
Create a sibling pane with the same geometry rule, preserve the caller's working directory, and keep user focus unchanged (or, from outside a managed pane, use an explicit target pane and `--cwd` as above):
